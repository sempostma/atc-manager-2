...
import { sendMessageError, sendMessageWarning, sendMessageInfo } from '../components/GameMessages/GameMessages';
...
const promiseMeATimelapse = timelapse => new Promise((res, rej) => {
...
class TimelapsePlaybackStore extends EventEmitter {
  ...
  scrub(frames) {
    ...
    if (frames >= this.timelapse.patches.length) throw 'Not enough frames in timelapse.';
    this.index = frames;
    GameStore.loadJson(this.states[this.index]);
    GameStore.emit('change');
    this.emit('change');
