...
class GameStore extends EventEmitter {
  ...
  adjustZoom = delta => {
    this.zoom = Math.min(config.maxZoom, Math.max(config.minZoom, this.zoom + this.zoom * (delta * config.zoomSpeed)));
    this.emit('change');
  }
  ...
  newPlaneVFREnroute = () => {
    ...
    const x = side === 1 ? config.width : side === 3 ? 0 : Math.random() * config.width;
    const y = side === 0 ? config.height : side === 2 ? 0 : Math.random() * config.height;

    const mx = config.width / 2;
    const my = config.height / 2;

    const tgtSides = [0, 1, 2, 3];
    tgtSides.splice(tgtSides.indexOf(side), 1);
    const tgtside = tgtSides.sort((a) => Math.random() - .5).pop();
    const xt = tgtside === 1 ? config.width : tgtside === 3 ? 0 : .5 * config.width;
    const yt = tgtside === 0 ? config.height : tgtside === 2 ? 0 : .5 * config.height;
  ...
  planeUpdate = (airplane, i) => {
    ...
    const shouldFindHeadingAround = (airplane.routeType === routeTypes.VFR_ENROUTE)
      || (airplane.routeType === routeTypes.VFR_OUTBOUND && airplane.tgtVfrState === VFRStates.OWN_DISCRETION && airplane.altitude > 800 + this.airport.elevation);

    const outsideControlAreaRadius = Math.pow(airplane.x - (this.airport.x + config.width / 2), 2)
      + Math.pow(airplane.y - (this.airport.y + config.height / 2), 2) > Math.pow(config.controlAreaRadius + 5, 2);
    ...
    if (shouldFindHeadingAround) {
      tgtHeading = findHdgAround(
        ...
        airplane.x,
        airplane.y,
        headingTo(airplane.x, airplane.y, this.callsignsPos[airplane.tgtDirection].x, this.callsignsPos[airplane.tgtDirection].y),
    ...
    if (airplane.routeType === routeTypes.OUTBOUND || airplane.routeType === routeTypes.ENROUTE
      || airplane.routeType === routeTypes.VFR_OUTBOUND || airplane.routeType === routeTypes.VFR_ENROUTE) {
    ...
    function vfrPattern(rwyPos) {
      const rwyElev = rwyPos.ref.name1 === airplane.tgtDirection ? rwyPos.ref.elevation1 : rwyPos.ref.elevation2;
      const rwyHdg = rwyPos.ref.name1 === airplane.tgtDirection ? rwyPos.ref.hdg1 : rwyPos.ref.hdg2;
      const hdgToRwy = (Math.atan2(rwyPos.x - airplane.x, rwyPos.y - airplane.y) * 180 / Math.PI + 360) % 360;
      const distanceToRwyPos = Math.sqrt(Math.pow(rwyPos.x - airplane.x, 2) + Math.pow(rwyPos.y - airplane.y, 2));
      const deg = angleDelta(rwyHdg, hdgToRwy);
      const within45DegILS = Math.abs(deg) < 45;
      const within45DegILSOpposite = Math.abs(angleDelta(rwyHdg, hdgToRwy + 180)) < 45;
      const rwyHdgDelta = angleDelta(rwyHdg, airplane.heading) * trafficDirectionMult;
      const perpendicularDistance = parrelelLinesDistance(airplane.x, airplane.y, rwyPos.x, rwyPos.y, rwyHdg);
      const parrelelDistance = parrelelLinesDistance(airplane.x, airplane.y, rwyPos.x, rwyPos.y, rwyHdg + 90);
      const rwyMiddleX = config.width / 2 + this.airport.x + rwyPos.ref.x;
      const rwyMiddleY = config.height / 2 + this.airport.y + rwyPos.ref.y;
      const goodSideOfRwy = angleDelta(hdgToRwy, rwyHdg) * trafficDirectionMult >= 0;

      const perpendicularDistMin = 12;
      const perpendicularDistMax = 30;
      const perpendicularDistMaxPreffered = 17;

      if (Airplane.isVFR(airplane) && airplane.altitude > 1010 + rwyElev
        && (airplane.routeType === routeTypes.VFR_INBOUND || airplane.routeType === routeTypes.VFR_INBOUND_TG)) {

        if (airplane.tgtVfrState === VFRStates.STRAIGHT_IN) {
          const vector = hdgToVector(rwyHdg);
          const lineupVector = [vector[0] * config.inboundPatternEntryDistance, vector[1] * config.inboundPatternEntryDistance];

          const entry1 = [rwyMiddleX - lineupVector[1] * .5 - lineupVector[0], rwyMiddleY + lineupVector[0] * .5 - lineupVector[1]];
          const entry2 = [rwyMiddleX + lineupVector[1] * .5 - lineupVector[0], rwyMiddleY - lineupVector[0] * .5 - lineupVector[1]];
          const entry3 = [rwyMiddleX - lineupVector[0] * 1.5, rwyMiddleY - lineupVector[1] * 1.5];

          const entry1Dist = Math.sqrt(Math.pow(entry1[0] - airplane.x, 2) + Math.pow(entry1[1] - airplane.y, 2));
          const entry2Dist = Math.sqrt(Math.pow(entry2[0] - airplane.x, 2) + Math.pow(entry2[1] - airplane.y, 2));
          const entry3Dist = Math.sqrt(Math.pow(entry3[0] - airplane.x, 2) + Math.pow(entry3[1] - airplane.y, 2));
          ...
          const vector = hdgToVector(rwyHdg);
          const entry1 = [rwyMiddleX - vector[1] * config.inboundPatternEntryDistance, rwyMiddleY + vector[0] * config.inboundPatternEntryDistance];
          const entry2 = [rwyMiddleX + vector[1] * config.inboundPatternEntryDistance, rwyMiddleY - vector[0] * config.inboundPatternEntryDistance];
          ...
          if (perpendicularDistance <= perpendicularDistMax && goodSideOfRwy && perpendicularDistance >= perpendicularDistMin) {
            ...
            if (perpendicularDistance < perpendicularDistMin || goodSideOfRwy === false)
              tgtHeading = wrapHeadig(rwyHdg + 135 * trafficDirectionMult);
            else if (perpendicularDistance > perpendicularDistMaxPreffered && goodSideOfRwy)
              tgtHeading = wrapHeadig(rwyHdg + 225 * trafficDirectionMult);
            else
              tgtHeading = wrapHeadig(rwyHdg + 180 * trafficDirectionMult);
            tgtSpeed = Math.min(model.topSpeed, config.maxTrafficPatternSpeed);
          } else if (distanceToRwyPos < config.inboundPatternEntryDistance || entry1Dist < 5 || entry2Dist < 5) {
      ...
      let final = (within45DegILS && Math.abs(deg) < 91) || airplane.altitude < 790 + rwyElev;
      ...
      // upwind
      if (validLeg(VFRStates.FINAL, final && Math.abs(rwyHdgDelta) < 91 && airplane.dirty)) { // final
        legs[VFRStates.FINAL]();
      } else if (validLeg(VFRStates.BASE, within45DegILS && parrelelDistance > 25)) { // base
        legs[VFRStates.BASE]();
      } else if (validLeg(VFRStates.DOWNWIND, /*Math.abs(rwyHdgDelta) < 181 && Math.abs(rwyHdgDelta) > 89 && */ perpendicularDistance > perpendicularDistMin)) { // downwind
        legs[VFRStates.DOWNWIND]();
      } else if (validLeg(VFRStates.CROSSWIND, rwyHdgDelta < 91 && rwyHdgDelta > -1 && within45DegILSOpposite && airplane.altitude + rwyElev > 800 && distanceToRwyPos > 20)) { // crosswind
        legs[VFRStates.CROSSWIND]();
      } else if (validLeg(VFRStates.UPWIND, rwyHdgDelta < 1 && rwyHdgDelta > -1)) { // upwind
        ...
        for (let i = 0; i < 9; i++) {
          ...
          if (sameIdentity && !sameIdentity[communications.getCallsign(airplane, true)]) {
            ...
            for (const key in sameIdentity) {
              ...
              if (xd * xd + yd * yd < 1) {
                if (xd * xd + yd * yd >= oneMileByThreeMileRuleDistance && (Airplane.isVFR(airplane) || Airplane.isVFR(oa))) return;
    ...
    function tryLand(rwyPos) {
      ...
      const tooHigh = airplane.altitude > (distance * config.ilsSlopeSteepness * 2 + (500 + rwyElev)/* safety */);
      distance = isFinite(distance) ? distance : .1;
      landing = true;

      const isTouchAndGo = airplane.routeType === routeTypes.VFR_CLOSED_PATTERN_TG
...
const legsOrder = Object.assign({}, ...['UPWIND', 'CROSSWIND', 'DOWNWIND', 'BASE', 'FINAL', 'RWY']
  .map((l, i) => ({ [VFRStates[l]]: i })));
