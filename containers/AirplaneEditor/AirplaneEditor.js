...
class AirplaneEditor extends Component {
  constructor(props) {
    ...
    this.state = {
      json: '',
      plane: null,
      planesSet: Object.assign({}, ...(loadState().customAirplanes || []).map(x => ({ [x.id]: x }))),
      debouncing: false,
      rawJSON: false,
    };
  }
  ...
  handleSaveClick = e => {
    if (this.state.plane === null) return sendMessageError('Please submit a valid save file');
  ...
  handlePlaneDeleteClick = e => {
    if (this.state.plane === null) return sendMessageWarning('No plane is selected');
    const planesSet = this.state.planesSet;
        ...
        <div className="panel">
          ...
          <select onInput={this.handleInputChanged}>
            ...
            {Object.keys(airplanesById).map((id, i) =>
              <option key={i} value={id} selected={eq(airplanesById[id], this.state.plane)}>{airplanesById[id].name}</option>
            )}
          </select><br /><br />
          <button onClick={this.handleNewPlaneClick}>New</button>
          {
            this.state.plane
              ? <button onClick={this.handlePlaneDeleteClick}>{ defaultAirplanes.map(x => x.id).includes(this.state.plane.id) 
                ? 'Reset'
                : 'Remove' }</button>
              : null
          }
          ...
          <input onChange={this.readFromFile} id="saveseditor" className="inputfile" type="file" accept=".json" disabled={!this.state.plane || this.state.plane.name === ''} />
          ...
          <button disabled={this.state.debouncing || this.state.plane === null} onClick={this.handleSaveClick}>{this.state.debouncing ? <FaSpinner className="spinner" /> : <FaPaperPlane />} Save</button>
        </div>
        <div className="panel">
          <h4 style="margin-bottom: 5px;">Submit for approval</h4>
          <small style="display: block; margin-bottom: 20px;">Submit currently selected airplane for approval.</small>
          
          <form action="https://getsimpleform.com/messages?form_api_token=edde415b219f71f64840e6a3dbd3ff7d" 
            style="margin-bottom: 0;" method="post" redirect={ url + '/#/editor/airplane-submission-success'}>
            <div>Email*</div>
            <input type="email" name="email" required></input>
            <input name="content_type" type="hidden" value="plane"></input>
            <input name="content" type="hidden" value={this.state.plane && JSON.stringify(this.state.plane, null, 4)}></input>
            <br />
            <br />
            <div>Sources*</div>
            <small>Links or text pointing to sources you have used to create or edit an airplane so we can verify the data.</small>
            <textarea name="message" required minLength="20"></textarea>
            <br />
            <br />
            <button disabled={this.state.debouncing || this.state.plane === null} type="submit">Submit</button>
          </form>
        </div>
      </div>
    );
