...
import { refresh, operatorsById, defaultOperators, operators } from '../../lib/airplane-library/airplane-library';
...
class OperatorEditor extends Component {
  constructor(props) {
    ...
    this.state = {
      json: '',
      plane: null,
      operatorsSet: Object.assign({}, ...(loadState().customOperators || []).map(x => ({ [x.id]: x }))),
      debouncing: false,
      rawJSON: false,
    };
  }
  ...
  handleSaveClick = e => {
    if (this.state.operator === null) return sendMessageError('Please submit a valid save file');
  ...
  handleOperatorDeleteClick = e => {
    if (this.state.operator === null) return sendMessageWarning('No operator is selected');
    const operatorsSet = this.state.operatorsSet;
  ...
  render() {
    const url = typeof window !== 'undefined' ? window.location.origin : 'https://esstudio.site/atc-manager-2';
        ...
        <div className="panel">
          ...
          <select onInput={this.handleInputChanged}>
            ...
            {Object.keys(operatorsById).map((id, i) =>
              <option key={i} value={id} selected={eq(operatorsById[id], this.state.operator)}>{operatorsById[id].name}</option>
            )}
          </select><br /><br />
          <button onClick={this.handleNewOperatorClick}>New</button>
          {
            this.state.operator
              ? <button onClick={this.handleOperatorDeleteClick}>{ defaultOperators.map(x => x.id).includes(this.state.operator.id) 
                ? 'Reset'
                : 'Remove' }</button>
              : null
          }
          ...
          <input onChange={this.readFromFile} id="saveseditor" className="inputfile" type="file" accept=".json" disabled={!this.state.operator || this.state.operator.name === ''} />
          ...
          <button disabled={this.state.debouncing || this.state.operator === null} onClick={this.handleSaveClick}>{this.state.debouncing ? <FaSpinner className="spinner" /> : <FaPaperPlane />} Save</button>
        </div>
        <div className="panel">
          <h4 style="margin-bottom: 5px;">Submit for approval</h4>
          <small style="display: block; margin-bottom: 20px;">Submit currently selected operator for approval.</small>
          
          <form action="https://getsimpleform.com/messages?form_api_token=edde415b219f71f64840e6a3dbd3ff7d" 
            style="margin-bottom: 0;" method="post" redirect={ url + '/#/editor/operator-submission-success'}>
            <div>Email*</div>
            <input type="email" name="email" required></input>
            <input name="content_type" type="hidden" value="operator"></input>
            <input name="content" type="hidden" value={this.state.operator && JSON.stringify(this.state.operator, null, 4)}></input>
            <br />
            <br />
            <div>Sources*</div>
            <small>Links or text pointing to sources you have used to create or edit an operator so we can verify the data.</small>
            <textarea name="message" required minLength="20"></textarea>
            <br />
            <br />
            <button disabled={this.state.debouncing || this.state.operator === null} type="submit">Submit</button>
          </form>
        </div>
      </div>
    );
