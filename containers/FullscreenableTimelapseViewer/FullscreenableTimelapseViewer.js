...
import { FaAngleDoubleRight, FaPlay, FaStop, FaAngleRight, FaCompress, FaExpand, FaArrowsAlt, FaSearch } from 'react-icons/fa/index.mjs';
...
class TimelapseViewer extends Component {
  ...
  componentWillMount() {
    ...
    if (typeof window !== 'undefined') document.addEventListener('mouseup', this.handleScrubDone);
    if (typeof window !== 'undefined') document.addEventListener('touchend', this.handleScrubDone);
  }

  componentWillUnmount() {
    TimelapsePlaybackStore.removeListener('change', this.handleTimelapsePlaybackStoreChange);
    if (typeof window !== 'undefined') document.removeEventListener('mouseup', this.handleScrubDone);
    if (typeof window !== 'undefined') document.removeEventListener('touchend', this.handleScrubDone);
  }
  ...
  render() {
    ...
    return (
      <div className="TimelapseViewer">
        <div onMouseMove={this.handleMouseMove} class={`video-wrapper ${this.state.show ? 'show' : ''}`}>
          <SvgRadar timelapse className="video" onZoom={this.handleZoom} onClick={this.handleSVGClick} cmd={this.state.cmd} />
          <div className="video-ui">
            <button onClick={this.handleTogglePlay} className="video-play">
              {TimelapsePlaybackStore.interval !== null ? <FaStop /> : <FaPlay />}
            </button>
            <div className="video-speed">
              ...
              <button className="video-speed-btn">
                { this.state.rawSpeed > 0 ? <FaAngleDoubleRight /> : <FaAngleRight /> }
              </button>
              <input onInput={this.handleSpeed} value={this.state.rawSpeed} step="1" min="-9" max="9" orient="vertical" type="range" className="video-speed-slider" />
            </div>
            <div className="video-zoom">
              <button className="video-zoom-btn">
                <FaSearch />
              </button>
              <input onInput={this.handleZoomSlider} value={this.state.rawZoom} step="0.1" min="1" max={Math.sqrt(10)} orient="vertical" type="range" className="video-zoom-slider" />
            </div>
            <div className="range-slider video-visual-progress">
              <input
                ...
                max={len - 1}
                step="1" />
              <span style="width: 100px" className="range-slider__value">{gamestoreFramesTimeFmt(index)}/{gamestoreFramesTimeFmt(len - 1)}</span>
            </div>
            <button onClick={this.props.toggleFullscreen} className="video-fullscreen">
