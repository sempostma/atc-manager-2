...
export const parseRoute = (txt, allowedCallsignsSet) => {
  ...
  const commands = value.map(x => x.match(reg))
...
const legScore = (airplane, leg, callsignPositions) => {
  ...
  return parrelelLinesDistance(airplane.x, airplane.y, to.x, to.y, legHdg) + Math.abs(airplane.heading, legHdg) / 10;
...
export const mostSuitableLeg = (airplane, sidOrStar, callsignPositions) => {
  ...
  const orderedBySuitability = legs.slice(0).sort((leg1, leg2) => {
    return legScore(airplane, leg2, callsignPositions)
      - legScore(airplane, leg1, callsignPositions);
  });

  let best = orderedBySuitability.pop();
  while (distSq(airplane, callsignPositions[best.to.dir]) < distBeforeTurningOntoNewLeg(airplane)) {
...
window.distBeforeTurningOntoNewLeg = distBeforeTurningOntoNewLeg;
