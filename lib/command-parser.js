  ...
  tgtVfrState: {
    ...
    action: (cmd, [direction]) => {
      ...
      const allowedStatesNames = allowedStates.map(x =>
        ({ val: x, name: VFRStates[x] }));
      const state = allowedStatesNames.find(state =>
        state.name.indexOf(direction.toLowerCase()) !== -1);
...
export const parse = (traffic, text, oldCmd) => {
  ...
  let airplane = traffic.find(t =>
    callsign === communications.getCallsign(t, true));
  if (!airplane) {
    airplane = oldCmd.tgt;
    command = split.join(' ');
  }
  ...
  const cmd = {
    tgt: airplane,
    direction: typeof airplane.tgtDirection === 'string'
      ? airplane.tgtDirection
      : null,
    altitude: airplane.tgtAltitude,
    heading: typeof airplane.tgtDirection === 'number'
      ? airplane.tgtDirection
      : null,
