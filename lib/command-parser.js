import Airplane, { emitter } from './airplane';
import communications from './communications';
import { allowedVFRStates, VFRStates, airplanesById } from './airplane-library/airplane-library';
...
const cmds = {
  turn: {
    ...
    action: (cmd, [turn, direction, heading, deg]) => mkCmd(cmd, { heading: +deg % 360 })
  },
  altitude: {
    regex: /(climb|descend|alt)( and)?( maintain)? (FL ?[0-9]{1,3}|[0-9]{1,6})/i,
    action: (cmd, [climbDescend, and, maintain, altitude]) => {
      if (altitude.startsWith('FL')) altitude = +altitude.replace('FL', '').trim() * 100;
  ...
  speed: {
    ...
    action: (cmd, [reduceIncrease, to, speed]) => {
      ...
      const clampedSpd = Math.min(model.topSpeed, Math.max(model.minSpeed, +speed));
  ...
  tgtVfrState: {
    ...
    action: (cmd, [direction]) => {
      ...
      const allowedStatesNames = allowedStates.map(x => ({ val: x, name: VFRStates[x] }));
      const state = allowedStatesNames.find(state => state.name.indexOf(direction.toLowerCase()) !== -1);
...
export const parse = (traffic, text, oldCmd) => {
  ...
  let airplane = traffic.find(t => callsign === communications.getCallsign(t, true));
  if (!airplane) {
    airplane = oldCmd.tgt;
    command = split.join(' ');
  }
