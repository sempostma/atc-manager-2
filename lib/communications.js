...
class Communications extends EventEmitter {
  ...
  getCallsign(airplane, short) {
    ...
    if (operator) {
      const cs = operator[short ? 'callsign' : 'shortName'] + (short ? '' : ' ') + airplane.flight;
  ...
  getCommandText(cmd, winddir, windspd, map, callsignsPos) {
    ...
    if (cmd.takeoff && cmd.tgt.rwy) {
      if (cmd.tgt.routeType === routeTypes.VFR_OUTBOUND) {
        txt.push(`takeoff runway ${cmd.tgt.rwy}, the wind is ${winddir.toFixed(0)} at ${windspd.toFixed(0)} knots, depart ${VFRStates[cmd.tgtVfrState]}`);
      } else if (Airplane.isVFR(cmd.tgt)) {
        txt.push(`takeoff runway ${cmd.tgt.rwy}, the wind is ${winddir.toFixed(0)} at ${windspd.toFixed(0)} knots, remain in pattern`);
      } else {
        txt.push(`takeoff runway ${cmd.tgt.rwy}, the wind is ${winddir.toFixed(0)} at ${windspd.toFixed(0)} knots`);
      }
    }
    if (cmd.goAround) {
      txt.push(`go around ${cmd.tgt.rwy}, fly runway heading, await further instructions`);
    }
    if (cmd.goAroundVFR) {
    ...
    if (cmd.direction !== undefined && cmd.direction !== cmd.tgt.tgtDirection && csPos && csPos.ref) {
    ...
    if (cmd.altitude && cmd.altitude !== cmd.tgt.tgtAltitude && !Airplane.isVFR(cmd.tgt)) {
