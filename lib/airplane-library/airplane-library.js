...
export const allowedVFRStates = airplane => {
  ...
  case routeTypes.VFR_CLOSED_PATTERN:
    return [
      VFRStates.RWY, VFRStates.UPWIND, VFRStates.CROSSWIND, VFRStates.DOWNWIND, VFRStates.BASE,
      VFRStates.FINAL
    ];
  ...
  case routeTypes.VFR_OUTBOUND:
    ...
    return [
      VFRStates.STRAIGHT_OUT, VFRStates.EXIT_45_DEG_OUT, VFRStates.OWN_DISCRETION
    ];
  case routeTypes.VFR_INBOUND:
    return [
      VFRStates.RWY, VFRStates.UPWIND, VFRStates.CROSSWIND, VFRStates.DOWNWIND, VFRStates.BASE,
      VFRStates.FINAL, VFRStates.STRAIGHT_IN
    ];
...
export const rndAirplane = ga => {
  const aps = airplanes.filter(ap => ga ? ap.ga > 0 : ap.commercial > 0);
  let counter = 0;
  const airplaneRatios = aps.map(plane => ({ airplane: plane, count: counter += plane.rarity }));
...
export const rndOperator = airplane => {
  ...
  let counter = 0;
  const operatorRatios = operators.map(op => ({ opid: op.id, count: counter += op.rarity }));
