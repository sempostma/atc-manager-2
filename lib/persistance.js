...
export const loadState = function () {
  if (val === null) {
    ...
    const gamesJson = typeof window !== 'undefined' && localStorage.getItem(name + '-games') || '{}';
    const introTutorialJson = typeof window !== 'undefined' && localStorage.getItem(name + '-intro-tutorial') || 'false';
    const timelapsesJson = typeof window !== 'undefined' && localStorage.getItem(name + '-timelapses') || '{}';
    const settingsJson = typeof window !== 'undefined' && localStorage.getItem(name + '-settings') || '{}';
    return {
...
export const decimalFormatter = decimals => (key, val) => !nullOrUndefined(val) && val.toFixed ? +val.toFixed(decimals) : val;
