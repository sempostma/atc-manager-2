...
export const loadMap = key => {
  Object.keys(maps[key].waypoints).map(k => maps[key].waypoints[k])
...
export const findHdgAround = (centerx, centery, radius, px, py, hdg, tx, ty) => {
...
export const hdgToVector = hdg => [Math.sin(hdg * Math.PI / 180), Math.cos(hdg * Math.PI / 180)];
...
export const parrelelLinesDistance = (px1, py1, px2, py2, hdg) => {
  const distanceBetweenPoints = Math.sqrt(Math.pow(px1 - px2, 2) + Math.pow(py1 - py2, 2));
...
export const activeRwys = (airport, winddir) => {
  ...
  if (ru) { // IF runway usage spec is available
  ...
  } else { // use runways that point into the wind
    const aRwys = [];
    for (let i = 0; i < airport.runways.length; i++) {
      const rwy = airport.runways[i];
      aRwys.push(Math.abs(winddir - rwy.hdg1) < Math.abs(winddir - rwy.hdg2) ? rwy.name1 : rwy.name2);
    }
    return aRwys;
  }
};
...
export const callsignPositions = (map, waypoints, width, height, parsedSids, parsedStars) => {
  return Object.assign({}, ...Object.keys(waypoints).map(k => {
    const ref = waypoints[k];
    return { [k]: { ref, x: ref.x, y: ref.y } };
  }),
  { [map.airport.callsign]: { ref: map.airport, x: width / 2 + map.airport.x, y: height / 2 + map.airport.y } },
...
export const setCallsigns = (map, waypoints, parsedSids, parsedStars) =>
  Object.assign({}, waypoints, { [map.airport.callsign]: map.airport },
...
export const testMSAViolation = (map, airplane) => {
  return map.msa.polygons.some(({ vertices, alt }) => {
    return polygonInside([airplane.x, airplane.y], vertices) && airplane.altitude < alt;
  });
};
