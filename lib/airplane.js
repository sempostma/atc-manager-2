import { airplanes, routeTypes, airplanesById } from './airplane-library';
...
export default class Airplane {
  ...
  getTurningRate = () => {
    const model = airplanesById[this.typeId];
    const turningRate = model.turningRate;

    if (this.speed <= model.minSpeed) 
      return turningRate[0];
    else if (this.speed <= 250) 
      return turningRate[0] + (this.speed - model.minSpeed) / (250 - model.minSpeed) * (turningRate[1] - turningRate[0]);
    else 
      return turningRate[1] + (this.speed - 250) / (model.topSpeed - 250) * (turningRate[2] - turningRate[1]);
  }
