import { airplanes, routeTypes, airplanesById, VFRStates, rndAirplane, rndOperator } from './airplane-library/airplane-library';
...
const create = (x, y, heading, routeType, ga, map) => {
  let airplane = rndAirplane(ga);
  let operatorId = airplane.operators.length > 0
    ? rndOperator(airplane)
    : null;
...
const headingToMiddle = (x, y) => Math.floor(headingTo(x, y, config.width / 2, config.height / 2) + 360) % 360;

const headingFromMiddle = (x, y) => Math.floor(headingTo(config.width / 2, config.height / 2, x, y) + 360) % 360;
...
export default class Airplane {
  ...
  static createInbound(x, y, heading, map) {
    const airplane = Airplane.create(x, y, heading, routeTypes.INBOUND, false, map);
    airplane.altitude = airplane.tgtAltitude = Math.max(airplane.altitude, getMSABaseThousands(map.msa.base, headingFromMiddle(x, y)));
    return airplane;
  }

  static createOutbound(x, y, heading, rwy, outboundWaypoint, map) {
    let airplane = create(x, y, heading, routeTypes.OUTBOUND, false, map);
    const hdgFromMid = headingFromMiddle(map.waypoints[outboundWaypoint].x, map.waypoints[outboundWaypoint].y);
    airplane.tgtAltitude = Math.max(airplane.tgtAltitude, getMSABaseThousands(map.msa.base, hdgFromMid));
    airplane.altitude = 0;
    airplane.speed = 0;
  ...
  static createVFROutbound(x, y, heading, rwy, outboundWaypoint, map) {
  ...
  static createVFRClosedPattern(x, y, heading, rwy, touchandgo, map) {
    let airplane = create(x, y, heading, touchandgo ? routeTypes.VFR_CLOSED_PATTERN_TG : routeTypes.VFR_CLOSED_PATTERN, true, map);
    airplane.altitude = 0;
    airplane.speed = 0;
    airplane.dirty = false;
    airplane.rwy = rwy;
  ...
  static createVFRInbound(x, y, heading, touchandgo, map) {
    let airplane = create(x, y, heading, touchandgo ? routeTypes.VFR_INBOUND_TG : routeTypes.VFR_INBOUND, true, map);
    airplane.altitude = airplane.tgtAltitude = Math.floor(Math.random() * 3 + 2) * 1000;
    airplane.altitude = airplane.tgtAltitude = Math.max(airplane.altitude, getMSABaseThousands(map.msa.base, headingFromMiddle(x, y)));
    airplane.regNum = regNums[regNumCounter++];
    airplane.tgtVfrState = VFRStates.RWY;
