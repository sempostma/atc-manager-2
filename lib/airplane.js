...
const create = (x, y, heading, routeType, ga) => {
...
export default class Airplane {
  ...
  static create = (x, y, heading, routeType, ga) => {
    const airplane = create(x, y, heading, routeType);
  ...
  static createOutbound(x, y, heading, rwy, outboundWaypoint) {
    let airplane = create(x, y, heading, routeTypes.OUTBOUND);
    airplane.altitude = 0;
    airplane.speed = 0;
  ...
  static createVFROutbound(x, y, heading, rwy, outboundWaypoint) {
    let airplane = create(x, y, heading, routeTypes.VFR_OUTBOUND, true);
    airplane.altitude = 0;
    airplane.speed = 0;
    airplane.dirty = false;
    airplane.rwy = rwy;
  ...
  static createEnroute(x, y, heading, outboundWaypoint) {
    let airplane = create(x, y, heading, routeTypes.ENROUTE);
    airplane.outboundWaypoint = outboundWaypoint;
    emitter.emit('create', airplane);
    return airplane;
  }
  ...
  static createVFRClosedPattern(x, y, heading, rwy, touchandgo) {
    let airplane = create(x, y, heading, touchandgo ? routeTypes.VFR_CLOSED_PATTERN_TG : routeTypes.VFR_CLOSED_PATTERN, true);
    airplane.altitude = 0;
    airplane.speed = 0;
    airplane.dirty = false;
    airplane.rwy = rwy;
  ...
  static createVFREnroute(x, y, heading, outboundWaypoint) {
    let airplane = create(x, y, heading, routeTypes.VFR_ENROUTE, true);
  ...
  static createVFRInbound(x, y, heading, touchandgo) {
    let airplane = create(x, y, heading, touchandgo ? routeTypes.VFR_INBOUND_TG : routeTypes.VFR_INBOUND, true);
    airplane.altitude = airplane.tgtAltitude = Math.floor(Math.random() * 3 + 2) * 1000;
    airplane.regNum = regNums[regNumCounter++];
    airplane.tgtVfrState = VFRStates.RWY;
    regNumCounter %= 2000;
