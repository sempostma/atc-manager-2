import { routeTypes, airplanesById, VFRStates, rndAirplane, rndOperator } from './airplane-library/airplane-library';
...
const create = (x, y, heading, routeType, ga, map) => {
  ...
  let altitudeThousands = Math.floor(
    airplane.ceiling * .5 - Math.random() * airplane.ceiling * .2
  );
  let altitude = ((altitudeThousands % 2 === 0) === (heading < 180)
    ? altitudeThousands - 1
...
export default class Airplane {
  ...
  static getTurningRate = airplane => {
    ...
    if (airplane.speed <= model.minSpeed) {
      ...
      return turningRate[0] + (airplane.speed - model.minSpeed)
        / (250 - model.minSpeed) * (turningRate[1] - turningRate[0]);
    }
    else {
      return turningRate[1] + (airplane.speed - 250)
        / (model.topSpeed - 250) * (turningRate[2] - turningRate[1]);
    }
  }

  static createInbound(x, y, heading, map) {
    const airplane = Airplane.create(
      x, y, heading, routeTypes.INBOUND, false, map);

    const msaBaseThousands = getMSABaseThousands(
      map.msa.base, headingFromMiddle(x, y));

    airplane.altitude = airplane.tgtAltitude = Math.max(
      airplane.altitude, msaBaseThousands);
  ...
  static isVFR(airplane) {
    return airplane.tgtVfrState !== undefined
