...
const PlaneAlt = ({ airplane, tagName }) => {
  ...
  const isLandingOrApproaching = cs && cs.ref.type === idType.RWY && airplane.routeType === routeTypes.INBOUND;
  const landingDescend = isLandingOrApproaching && airplane.altitude < airplane.tgtAltitude;
  const TagName = tagName;
  const showTgt = Math.abs(airplane.tgtAltitude - airplane.altitude) > 100 && airplane.altitude && !landingDescend && !Airplane.isVFR(airplane);
  const tgt = showTgt
