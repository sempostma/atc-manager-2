...
const excerpt = (str, len) => str.length > len ? str.slice(0, len - 3).trim() + '...' : str;
...
class AtomFeed extends Component {
  ...
  parseAtomDocument = feed => {
    ...
    const items = Array.from(feed.getElementsByTagName('entry')).map(entry => ({
      ...
      content: excerpt(entry.getElementsByTagName('content')[0].textContent.replace(/<(?:.|\n)*?>/gm, '').replace(/[\s\r\n]+/gm, ' '), 100),
      link: entry.getElementsByTagName('link')[0].getAttribute('href'),
      id: entry.getElementsByTagName('id')[0].textContent,
      image: entry.getElementsByTagName('media:thumbnail')[0].getAttribute('url')
    }));
  ...
  render() {
    return (
      <div className="AtomFeed">
        ...
        { this.state.items.map(item => <AtomFeedItem {...item} key={item.id} />) }
      </div>
    );
  }
}
