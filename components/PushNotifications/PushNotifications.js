...
class PushNotifications extends Component {
            ...
            registration.pushManager.subscribe({
              ...
              fetch(config.subscriptionURL, {
                ...
                .then(json => {
                  ...
                  console.log('Successfully registered push notifications', obj);
                  this.setState({ status: status.REGISTERED, subscription });
                }).catch(err => {
                  throw err;
                });
  ...
  unsubscribe = () => {
    this.state.subscription.unsubscribe().then(succesfull => {
    ...
    return (
      <div className="PushNotifications" style={classList.join(' ')}>
        {this.showButtons() && <div>
          ...
          <button onClick={() => this.setState({ status: status.USER_REJECTED })}
            disabled={btnDisabled}>No, Leave me alone *squawks 7600*</button>
          <button onClick={this.state.cb} disabled={btnDisabled}>Yes, acknowledge handover</button>
        </div>}
        {this.state.status === status.REGISTERED && <div>
          We can ready you five by five. We'll keep you up to date.
          <button onClick={this.unsubscribe}>Terminate radio contact</button>
        </div>}
        {this.state.status === status.ERROR && <div>
          Whoops, something went wrong. *squawk 7700
        </div>}
        {this.state.status === status.USER_REJECTED && <div>
          Radio silence.
        </div>}
        {Object.keys(this.state.status).filter(key => status[key] === this.state.status).join('')}
      </div>
    );
  }
}
