...
class RadarTraffic extends Component {
  ...
  render() {
    ...
    const y = (config.height / 2 - airplane.y) * GameStore.zoom + config.height / 2;
    const x = (airplane.x - config.width / 2) * GameStore.zoom + config.width / 2;
    ...
    return <g className={`airplane ${routeTypes[airplane.routeType].replace(/ /g, '-')} ${this.props.cmd.tgt === airplane ? 'airplane-active' : 'airplane-inactive'}`}
      ...
      <rect width="5" height="5" x="-2.5" y="-2.5" fill="none" strokeWidth="2" />
      <line x1="0" y1="0" x2={ltx} y2={-lty} />
      <path stroke-dasharray={`${4 * GameStore.zoom}, ${5 * GameStore.zoom}`} d={path} />
